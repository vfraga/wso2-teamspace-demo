...
| `IS_ORG_HANDLE` | Root B2B tenant handle. Set to `teamspace` for tenant mode, leave empty for `carbon.super`. Also the tenant `setup_is.py` **creates**, so the tenant that exists and the tenant the services look under cannot disagree. An empty value still provisions `teamspace` — setup needs a domain to create the tenant with | *(empty)* |
| `IS_VERIFY_TLS` | Verify the IS TLS certificate. Applies to every outbound call, including OIDC discovery, the token endpoint and the JWKS fetch. Accepts `true`, `false`, or a **path to a CA bundle** — use the path form with the demo CA in [`pki/`](./pki), since Python trusts certifi rather than the macOS keychain. Off by default because the quickstart runs against WSO2's self-signed certificate; whenever it resolves to off, every service logs a warning saying so | `false` |
| `WSO2_IS_TEMPLATE_PATH` | Path to a clean, un-bootstrapped WSO2 IS 7.2.0 install. Cloned by the live E2E fixture and by `setup_secondary_is.py`. No default: the live suite skips without it and `setup_secondary_is.py` exits with an error, rather than either guessing a path | *(unset)* |
...
| `PORTAL_URL` | Public origin of the portal, used to build the OIDC redirect and post-logout URIs. Must match the callback registered by `setup_is.py`, which reads the same variable. Set this rather than `FLASK_HOST`/`FLASK_PORT` when the scheme or hostname changes | `http://<FLASK_HOST>:<FLASK_PORT>` |
...
| `IS_ADMIN_USERNAME` | Basic-auth user for the one management call the portal cannot route through the `/o` organization endpoint: assigning `teamspace-user` to a freshly provisioned SCIM agent, which lives in the root-level `AGENT` userstore (`webapp/is_operations.py`) | `$IS_TENANT_ADMIN_USERNAME@$IS_ORG_HANDLE` |
| `IS_ADMIN_PASSWORD` | Password for the above. Both default to the account `setup_is.py` provisions, so a stock demo needs neither. Set them only when that admin is not the one to use | `$IS_TENANT_ADMIN_PASSWORD` |
...
| `AGENT_SERVICE_URL` | Public origin of the AI Agent service. Also what `setup_is.py` registers as an allowed origin and callback, and what the agent derives `AGENT_REDIRECT_URI` from, so all three always agree | `http://localhost:8000` |
...
| `SERVICE_VERIFY_TLS` | TLS verification for the internal hops (portal to Business API, portal to agent, agent to Business API). Same three-state form and same default as `IS_VERIFY_TLS`. Inert in the quickstart, where those hops are plain HTTP; it matters under `TLS_CERT_DIR`, where they are served on demo-CA certificates that certifi does not trust. Point it at `pki/out/ca-bundle.crt` there | `false` |
| `JWT_CLOCK_SKEW_SECONDS` | Tolerance applied to `exp`, `nbf` and `iat` when verifying tokens. Non-zero by default because WSO2 IS and the services are different machines whenever the stack runs in containers, and a token issued a moment ahead of the verifier's clock would otherwise be rejected as "not yet valid" | `60` |
...
| `TLS_CERT_DIR` | Serve all three services over HTTPS using certificates from this directory (the layout `pki/generate.sh` produces). Unset means plain HTTP, which is what the quickstart assumes | *(unset)* |
| `API_WORKERS`, `AGENT_WORKERS`, `WEBAPP_WORKERS` | Gunicorn worker counts in production mode | `4` |
| `API_PORT`, `AGENT_PORT` | Ports `start.sh` binds the Business API and agent to. Nothing derives these from `BUSINESS_API_URL` / `AGENT_SERVICE_URL`, so changing one means updating that URL too | `9091`, `8000` |
| `DB_AUTO_CREATE` | Whether the Business API creates missing tables at startup | On, except under `FLASK_ENV=production` |
| `HOST_GATEWAY_IP` | Address the TLS overlay maps `identityserver.test` and `secondaryidp.test` to. Only needed where `host-gateway` does not reach the host — see [With TLS](#with-tls) | `host-gateway` |
...
| Variable | Description | Default |
| :--- | :--- | :--- |
| `FEDERATED_IS_BASE_URL` | Root URL of the federated WSO2 IS. `setup_idp_server.py` derives its SCIM2 and management endpoints from it, so this is the only host setting the federated IdP needs | `https://localhost:9444` |
| `FEDERATED_IDP_TENANT_DOMAIN` | Tenant the federated IdP is created under. The admin's login name, the management and SCIM endpoints, and the seeded users' email addresses are all derived from it | `worklink.com` |
| `FEDERATED_IDP_TENANT_ADMIN_USERNAME` | Local part of the federated tenant admin's login name | `teamspaceadmin` |
| `FEDERATED_IDP_TENANT_ADMIN_EMAIL` | The tenant owner's email *attribute*. Deliberately not derived from the tenant domain; the domain appears in the login name instead | `teamspaceadmin@mail.com` |
| `WORKLINK_IDP_LOGO_URL` | Logo on the federated IdP's hosted login page | `<CDN_IMG_BASE_URL>/worklink-idp-logo.svg` |
| `WORKLINK_IDP_FAVICON_URL` | Favicon on that page | `<CDN_IMG_BASE_URL>/worklink-idp-favicon.svg` |
| `WORKLINK_IDP_PRIMARY_COLOR` | Primary color of that page | `#155E75` |
| `WORKLINK_IDP_SECONDARY_COLOR` | Secondary color of that page | `#D7EEF5` |

With the defaults, the federated tenant admin is `teamspaceadmin@worklink.com`,
authenticated with `IS_TENANT_ADMIN_PASSWORD` — the same variable as the primary
IS — and the seeded test users are `john@worklink.com` and `tom@worklink.com`,
who get `FEDERATED_USER_PASSWORD` (see [Setup script
credentials](#setup-script-credentials)). The live E2E suite signs in as those
two accounts, so `FEDERATED_IDP_TENANT_DOMAIN` takes it with you.

The login page is branded separately from Teamspace on purpose: the SSO redirect
should visibly land on a different identity provider.

### Branding defaults

| Variable | Description | Default |
| :--- | :--- | :--- |
| `CDN_IMG_BASE_URL` | Where the demo's image assets are served from. Both setup scripts and the two defaults below derive their URLs from it, so one setting moves every asset | jsDelivr path in `common/config.py` |
| `DEFAULT_LOGO_URL` | Logo used when an organization has no custom branding | `<CDN_IMG_BASE_URL>/teamspace-logo.svg` |
| `DEFAULT_FAVICON_URL` | Favicon used when an organization has no custom branding | `<CDN_IMG_BASE_URL>/teamspace-favicon.svg` |
...
| `IS_TENANT_ADMIN_PASSWORD` | Password for the bootstrapped tenant admin, which is the portal login password | *(empty)* |
| `IS_TENANT_ADMIN_USERNAME` | Login name of that tenant admin. The full username is `$IS_TENANT_ADMIN_USERNAME@$IS_ORG_HANDLE`. Read by the portal too, for the root-tenant call above | `teamspaceadmin` |
| `IS_TENANT_ADMIN_EMAIL` | The tenant owner's email *attribute*. Deliberately not derived from `IS_ORG_HANDLE`: the domain belongs in the login name, not this address | `teamspaceadmin@mail.com` |
...
The script is idempotent. It creates the tenant named by `IS_ORG_HANDLE`
(default `teamspace`), sets branding, registers the B2B application, and
configures API resources, scopes and roles. On success it prints `CLIENT_ID`,
`CLIENT_SECRET` and `APP_ID`; copy those into `.env`.
...
The portal (5001) and the agent (8000) publish host ports. The agent has to:
the OBO consent popup is opened by the *browser* against `AGENT_REDIRECT_URI`,
and the compose network is not reachable from there. The Business API stays
internal; its `ports` entry is commented out in the compose file if you want it
...
> [!NOTE]
> `AGENT_REDIRECT_URI` has to name a host the *browser* can resolve, and it must
> match the callback `setup_is.py` registered from `AGENT_SERVICE_URL`. The
> defaults (`http://localhost:8000/callback`) work because port 8000 is
> published; the TLS overlay swaps both for `https://aiagent.test:8000`.

#### With TLS

Everything above runs over plain HTTP, which keeps the quickstart free of
certificates. To run the stack the way it is meant to be deployed — every hop
verified, including the container-to-container ones — generate the demo PKI and
add the overlay:

```bash
./pki/generate.sh
./pki/apply_to_is.sh --is-path /path/to/primary-wso2is   --cert identityserver
./pki/apply_to_is.sh --is-path /path/to/federated-wso2is --cert secondaryidp
```

Set `[server] hostname` in each IS `deployment.toml` to `identityserver.test` /
`secondaryidp.test`, restart both, and add to `/etc/hosts`:

```
127.0.0.1 identityserver.test secondaryidp.test flaskapp.test aiagent.test businessapi.test
```

Then, in containers. Both `-f` flags are required: the overlay only patches the
base file and does not describe a runnable stack on its own.
...
> [!IMPORTANT]
> On Rancher Desktop, Lima or Colima, set `HOST_GATEWAY_IP` first. The overlay
> maps `identityserver.test` and `secondaryidp.test` to `host-gateway`, which is
> right on Docker Desktop but resolves to the bridge gateway *inside* the VM
> elsewhere — where WSO2 IS is not listening, so every call to it is refused.
> Find the address `host.docker.internal` points at and use that:
>
> ```bash
> docker compose exec webapp getent hosts host.docker.internal   # e.g. 192.168.5.2
> ```
>
> `extra_hosts` accepts an IP or the literal `host-gateway`, so the alias cannot
> just be pointed at `host.docker.internal`.

Or on the host:

```bash
TLS_CERT_DIR=$PWD/pki/out SERVE_MODE=production bash start.sh
```

The host path needs the matching client-side settings in `.env`:
`IS_VERIFY_TLS` and `SERVICE_VERIFY_TLS` pointed at `$PWD/pki/out/ca-bundle.crt`,
and `PORTAL_URL`, `AGENT_SERVICE_URL` and `AGENT_REDIRECT_URI` on their
`https://<name>.test` names. Re-run `setup_is.py` afterwards so the registered
callbacks match what the browser will send.

Why the hostnames matter: WSO2 IS stamps its configured `[server] hostname` into
every URL it generates — OIDC discovery, `iss`, `jwks_uri` — regardless of the
Host header used to reach it. If the browser, the containers and IS itself do
not agree on one name, the issuer check fails on tokens that are otherwise
perfectly valid. Each generated certificate carries the `.test` name, the
compose service name, `localhost` and `host.docker.internal` in its SAN so the
same certificate is valid however the service is addressed. See
[`pki/README.md`](./pki/README.md).
...
Hermetic: no WSO2 IS, no Gemini key, no browser. 378 tests covering schemas,
...
The agent state store is exercised against both backends. The Redis half skips
when nothing answers at `REDIS_TEST_URL` (default `redis://127.0.0.1:6379/15`),
so the suite is still hermetic without one.
...
```bash
export WSO2_IS_TEMPLATE_PATH="/path/to/wso2is-7.2.0.42"
...
`.githooks/pre-commit` refuses any staged content containing an absolute home
directory (`/Users/...`, `/home/...`). Configuration belongs in the environment;
`setup_secondary_is.py` once shipped with its author's home directory baked in as
a default. Hooks are per-clone, so enable it once:

```bash
git config core.hooksPath .githooks
```

It inspects staged content rather than the working tree, so a partially-staged
file is judged by what actually goes in. Bypass a deliberate exception with
...
1. **TLS.** Verification against WSO2 IS is off by default (`IS_VERIFY_TLS`),
   for the self-signed certificate the quickstart runs against; each service
   logs a warning at startup saying so. [`pki/`](./pki) generates a local CA and
   per-service certificates so the whole stack can run verified instead — see
   [With TLS](#with-tls). In production, replace that demo CA with real
   certificates, keep `IS_VERIFY_TLS`/`SERVICE_VERIFY_TLS` pointed at the right
   trust store, and terminate TLS in front of the stack. Gunicorn should not be
   internet-facing.
