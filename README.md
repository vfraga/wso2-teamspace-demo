...
| `SERVICE_VERIFY_TLS` | TLS verification for the internal hops (portal to Business API, portal to agent, agent to Business API). Same three-state form as `IS_VERIFY_TLS`. Only relevant once those services are served over HTTPS | `true` |
...
The federated IdP's tenant admin is `teamspaceadmin@worklink.com`, authenticated with
`IS_TENANT_ADMIN_PASSWORD` — the same variable as the primary IS. Its seeded test
users get `FEDERATED_USER_PASSWORD` (see [Setup script
credentials](#setup-script-credentials)).
...
> `AGENT_REDIRECT_URI` set to match. The TLS overlay below already does both.
...
1. **TLS.** The quickstart disables verification (`IS_VERIFY_TLS=false`) for
   WSO2's self-signed certificate. [`pki/`](./pki) generates a local CA and
