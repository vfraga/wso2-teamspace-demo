...
| `IS_ADMIN_USERNAME` | Basic-auth user for the one management call the portal cannot route through the `/o` organization endpoint: assigning `teamspace-user` to a freshly provisioned SCIM agent, which lives in the root-level `AGENT` userstore (`webapp/is_operations.py`) | `teamspaceadmin@teamspace` |
| `IS_ADMIN_PASSWORD` | Password for the above. Set it whenever `IS_TENANT_ADMIN_PASSWORD` is not the built-in default, or that one role assignment 401s while everything else keeps working | `Admin123` |
...
Hermetic: no WSO2 IS, no Gemini key, no browser. 360 tests covering schemas,
